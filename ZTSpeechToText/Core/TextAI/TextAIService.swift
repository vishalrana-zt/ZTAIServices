...
nonisolated private func pageFieldFocus(_ page: UITargetPage, documentType: StructuredDocumentType) -> String {
    ...
    case .equipmentAsset:
        ...
        let tagHeuristicNote = documentType == .fireEquipment
            ...
            - In option menus/lists, selection marks can appear as stray OCR symbols near only one line (for example trailing or leading `•`, `o`, `/`, or `@` on one option while sibling options are plain text). Treat that locally unique mark as a selected indicator for that line and map it to the corresponding field (agent type, checklist item, or deficiency context).
            ...
            - Nameplates and manufacturer data plates often carry brand names (e.g. GLOBE, VICTAULIC, TYCO, VIKING, CENTRAL, RELIABLE, POTTER, NOTIFIER, ARMSTRONG), model/part codes (e.g. RCW, LF, OS&Y, PIV, BFP, PRV followed by alphanumeric text), serial/asset strings, pressure ratings (e.g. "300 PSI", "20 BAR"), and compliance marks (UL, FM, LISTED). Treat every one of these as extractable data, not noise — put pressure ratings and similar readings in `keyFacts` and/or `equipment[].condition` as appropriate.
            - Fire pump system nameplates come in two distinct forms — apply different model extraction logic for each:
              (A) **Centrifugal pump heads** (cast iron/bronze pump casings, labels from Peerless, Armstrong, Grundfos, Patterson, etc.): show a size designation in format NxNxNMF or NxNxN (e.g. "10X8X11MF", "6X4X10") indicating suction × discharge × impeller — this is the true model for `equipment[].model`. A separate purely numeric catalog/part number (e.g. "428358-000") goes in `keyFacts` only, never in `model`.
              (B) **Electric fire pump motors / inverter duty motors** (labels say "Inverter Duty Motor", "Fire Pump Motor", "Duty Motor", show HP/kW ratings, or are from GE, Nidec, WEG, Baldor, Regal-Beloit, Marathon, etc.): use an alphanumeric catalog model number that encodes motor specs — e.g. "200180P3CFP445TS-W40" encodes 200 HP, 3-phase, CFP duty, 445TS frame. This alphanumeric string IS the motor model; extract it into `equipment[].model`. It typically appears as the FIRST prominent token in the OCR or is labeled near "CATALOG NO.", "CAT. NO.", or "PART NO." at the top of the nameplate plate. If this full catalog string and short series codes (like "HEL", "WAO", or letter-only abbreviations) both appear near the "MODEL" label, always prefer the longer alphanumeric catalog string. Do NOT apply the NxNxNMF centrifugal pump rule to motor nameplates.
              If neither identifier is legible, leave `model` empty rather than fabricating one.
            - **Motor nameplate date semantics**: on electric motor nameplates (Inverter Duty Motor, Fire Pump Motor), a compact date adjacent to the motor specs (e.g. "23NOV22" appearing near "FIRE PUMP MOTOR" or within the spec block) is the **manufacture or ship date** — put it in `equipment[].dateOfManufacture`, NOT in `lastInspectionDate` or `installDate`. A date is a service/inspection date only when it appears on a separate inspection tag, service record, or is explicitly labeled as "SERVICE DATE", "INSPECTION DATE", "DATE SERVICED", etc.
            - When a <context> block is present and contains a "visualSelections" array, treat every entry with "selected": true as a high-confidence indicator that the corresponding OCR line was physically marked or punched. Use it to override or confirm your OCR-only interpretation — e.g. route a visually selected agent-type line to equipment[].agentType, a deficiency line to deficiencies[], or a checklist item to checklistItems[]. Important: a physical tag can only have ONE punch per date slot. If multiple years appear selected in visualSelections, use "derivedServiceDate" (when present in context) as the authoritative answer; if derivedServiceDate is absent and multiple years compete, pick the year that best aligns with other OCR context clues (handwritten notes, explicit date text nearby); if no clues exist, leave the date field empty rather than guessing. The same rule applies to multiple selected months. Entries with "hint": "lowConfidence" are weaker pixel-detection signals — use them only to break a tie when OCR text alone is ambiguous (e.g. multiple agent types visible with no other selection indicator); never use a lowConfidence hint to override a clear OCR signal.
            ...
            - When a <context> block contains "possiblyPunchedAbsentFromOCR", these month abbreviations were completely absent from the OCR text — the most common cause is that the physical punch hole destroyed the printed text, making it unreadable by OCR. Key insight: the punched month is ABSENT from OCR (hole destroyed the ink), while non-punched months remain readable. This means a month listed in "possiblyPunchedAbsentFromOCR" is a stronger candidate for the actual punch than any month reported in "derivedServiceDate" or "visualSelections" (those months were readable precisely because they were NOT punched). If "possiblyPunchedAbsentFromOCR" is non-empty and exactly one absent month fits the tag context, prefer it over the derivedServiceDate month. If two absent months are listed (e.g. ["NOV","SEP"]), pick the one that best aligns with other date clues (handwritten notes, explicit year context); if neither can be distinguished, leave the month portion of lastInspectionDate empty rather than using the (likely incorrect) derivedServiceDate month.
...
nonisolated private func structuredExtractionRules(for documentType: StructuredDocumentType) -> String {
    ...
    case .fireEquipment:
        ...
        - Sticker/tag OCR heuristics: words like GLOBE, VICTAULIC, TYCO, VIKING, CENTRAL, RELIABLE, POTTER, NOTIFIER identify manufacturers. Words like RCW, LF, OS&Y, PIV, BFP, PRV followed by alphanumeric text are model/part numbers. Strings like 19S000RY, 1234ABC are serial/asset numbers. "300 PSI", "20 BAR", "175 PSI" are pressure ratings — put in keyFacts and equipment[].condition. "UL", "FM", "LISTED", "cUL", "CSA", "CE", "LPCB" are compliance/listing marks. "Calculated System", "Pipe Schedule System" identify the sprinkler system design type.
        - Listing marks: capture ALL listing marks visible on the label and combine them into `equipment[].manufacturerListing` as a comma-separated string (e.g. "UL Listed, FM Approved"). A UL mark often appears as the UL logo near text like "LISTED", "FIRE ALARM EQUIPMENT", "SIGNALING", or a category code (e.g. "3PBS") — treat any cluster of those words near a UL/cUL symbol as a UL listing even if the words are split across OCR lines. Do not drop any listing mark just because another is already captured.
        - Serial number cleanup: equipment serial numbers are alphanumeric strings without decimal points. If OCR introduces a spurious period inside a serial number (e.g. "D2022092104.90" should be "D202209210490"), remove the period and concatenate the digits. Only preserve a period in a serial if it is clearly a separator format used consistently on the label (e.g. "SN-123.456-A").
        ...
        - Treat locally unique OCR mark artifacts as selection hints in option lists: if one option line contains extra symbols (`•`, `o`, `/`, `@`) near the option text and neighboring options do not, interpret that marked line as selected instead of treating the symbol as noise.
...
actor CloudTextProvider: TextModelProvider {
    ...
    private func optimizedStructuredInputText(
        ...
        //
        // Customer (business cards, contact pages): name and company lines are plain
        // capitalized text with no digits, so the high-signal filter strips them entirely,
        // causing first name / last name / company to never reach the AI.
        //
