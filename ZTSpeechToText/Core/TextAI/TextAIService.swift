...
nonisolated private func pageFieldFocus(_ page: UITargetPage, documentType: StructuredDocumentType) -> String {
    ...
    case .equipmentAsset:
        ...
        let tagHeuristicNote = documentType == .fireEquipment
            ...
            - Fire pump nameplates often show TWO distinct identifiers near the "MODEL NO." label: (1) a pump **size designation** in the format NxNxNMF or NxNxN (e.g. "10X8X11MF", "6X4X10") indicating suction × discharge × impeller size — this is the true model and goes in `equipment[].model`; (2) a numeric **catalog/part number** (e.g. "428358-000", "428359-000") — this goes in `keyFacts` only, never in `model`. If only the catalog number is legible (size designation was unreadable), put the catalog number in `keyFacts` and leave `model` empty rather than using the catalog number as the model.
